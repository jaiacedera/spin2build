...
Anyone can browse and submit, without a visitor account. The four-step submission flow collects the project source and details, requires an HTTPS live demo URL, previews the same card/detail components used publicly, and publishes immediately. The landing page is embedded directly in a lazy-loaded, sandboxed iframe and scaled to fit the card. Project details allow interaction within the embedded page. No screenshot service, image uploads, or image storage bucket is needed. Sites can block embedding through X-Frame-Options or CSP frame-ancestors; the Open live demo link remains available because browsers do not reliably report these failures to the parent page. Same-origin demos are allowed because this app serves trusted application code; their scripts share the app's origin. When the gallery is itself embedded, it shows links instead of nesting more demo frames to prevent recursive galleries. Existing projects without an HTTPS demo URL show a placeholder. Existing demo URLs need no migration. For traffic protection and pagination, run `server/site-usage.sql` before deploying this version.

`POST /api/gallery` validates fields and HTTPS demo URLs, assigns `approved`, clears `featured`, and sets the creation time on the server. The server controls publication status and featured flags. Retrying the same submission ID does not create a duplicate or overwrite an existing build; retrying a hidden build does not republish it. `GET /api/gallery` returns approved projects only, newest first, in pages of 24 with `hasMore`; `page`, `search`, and `filter` select results. Community has search and All, From Spin2Build, Original Ideas, Featured, and Newest filters. My Builds has Drafts, In Progress, and Completed tabs. Local build status is independent of community moderation `status`.

During `npm run dev`, submissions persist as individual JSON files under `.gallery-data.local` (git-ignored and blocked from Vite's file server). No example builds are represented as community submissions. New submissions are public immediately. Privately change a file's `status` to `rejected` to hide a local build; set `featured` to `true` to feature it. Refresh the gallery after moderation. There is no public administration or update endpoint.

For a shared production gallery:

1. Run [server/gallery-schema.sql](server/gallery-schema.sql), then [server/site-usage.sql](server/site-usage.sql) in your Supabase SQL editor. See [traffic limits and deployment steps](DEPLOYMENT_LIMITS.md).
...
4. For builds already awaiting review, run [server/gallery-publish-pending.sql](server/gallery-publish-pending.sql) once to publish all pending entries. New submissions publish immediately. You can still manage visibility privately in the database by changing the row's top-level `status` to `approved` or `rejected`. Example queries are in the schema file. `project.featured` controls the Featured filter. Neither pending nor rejected records are returned publicly.

A single persistent Node server can instead set `GALLERY_LOCAL_DIR` to a durable directory. This file adapter is intended for one server; use the shared database for multiple instances. Local data is not automatically migrated to the production database. Production usage protection requires the shared Supabase configuration even when project records use a local file adapter.
